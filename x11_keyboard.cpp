...
static PyObject *init_x11(PyObject *self, PyObject *args) {
    ...
    if (display == NULL) {
        PyErr_SetString(PyExc_RuntimeError,
                        "Failed to open X display. Make sure DISPLAY is set.");
        return NULL;
    }
...
int safe_usleep(int base, int jitter) {
    ...
    if (total < 10000)
        total = 10000; // never below 10 ms
    if (total > 100000)
        total = 100000; // never above 100 ms
    return total;
}
...
static PyObject *type_text(PyObject *self, PyObject *args) {
    ...
    if (display == NULL) {
        PyErr_SetString(PyExc_RuntimeError,
                        "X11 not initialized. Call init() first.");
        return NULL;
    }
    ...
    for (size_t i = 0; i < strlen(text); i++) {
        ...
        usleep(safe_usleep(20000, 10000)); // Delay between keys
    }
...
static struct PyModuleDef keyboardmodule = {
    PyModuleDef_HEAD_INIT, "x11_keyboard", "X11 keyboard simulator using XTest",
    -1, KeyboardMethods};
